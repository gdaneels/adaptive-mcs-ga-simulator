...
def calculateReceived(data, exp, nrMotes=None, modulationConfig=None, ss=None):
    results = []
    for iteration in data:
        result = {'exp': getExperimentName(exp), 'val': None, 'iteration': iteration, 'iterationDir': os.path.dirname(iteration), 'motes': translateMotes[nrMotes], 'modulationConfig': translateConfigModulations[modulationConfig], 'ss': ss}
        nrReceived = 0
        for moteList in data[iteration]:
            if moteList['mote'] == 0:
                nrReceived += moteList['pktReceived']
...
def calculatePktArrivedToGen(data, exp, nrMotes=None, modulationConfig=None, ss=None):
    results = []
    for iteration in data:
        result = {'exp': getExperimentName(exp), 'val': None, 'iteration': iteration, 'motes': translateMotes[nrMotes], 'modulationConfig': translateConfigModulations[modulationConfig], 'ss': ss}
        nrPktArrivedToGen = 0
...
def calculateReliability(data, exp, modulationConfig=None, ss=None):
    results = []
    for iteration in data:
        for moteList in data[iteration]:
            # do not do this for root, only for source nodes
            ...
            if moteList['mote'] != 0 and moteList['reliability'] != None and moteList['reachRoot'] == 1:
                results.append({'exp': getExperimentName(exp), 'val': moteList['reliability'], 'iteration': iteration, 'mote': moteList['mote'], 'modulationConfig': translateConfigModulations[modulationConfig], 'ss': ss})
    return results
...
def plotBoxplotSeaborn(metric, data, sorter, outputDir='', xlabel=None):
    ...
    sns.despine()
    name = '{0}/boxplot-{1}-{2}.png'.format(outputDir, metric, datetime.datetime.now().strftime('%Y-%m-%d-%H-%M-%S'))
...
def plotLineSeaborn(metric, data, sorter, outputDir='', xlabel=None):
    ...
    sns.despine()
    name = '{0}/lineplot-{1}-{2}.png'.format(outputDir, metric, datetime.datetime.now().strftime('%Y-%m-%d-%H-%M-%S'))
    plt.ylabel(getLabelName(metric))
    if xlabel is not None:
        plt.xlabel(xlabel)
    # plt.legend(loc='upper left')
...
def plotMultipleBoxplotSeaborn(metric, data, sorter, outputDir='', xsplit=None, hue=None, xlabel=None, percentile=None):
    ...
    name = '{0}/multiple-boxplot-{1}-{2}.png'.format(outputDir, metric, datetime.datetime.now().strftime('%Y-%m-%d-%H-%M-%S'))
    plt.ylabel(getLabelName(metric))
    plt.xlabel(xlabel)
    # plt.legend(loc='upper left')
...
def plotCDFSeaborn(metric, data, sorter, outputDir='', xsplit=None, hue=None, xlabel=None, percentile=None):
    ...
    ax.set_xlabel('')
    sns.despine(ax=ax)
    name = '{0}/cdf-{1}-{2}.png'.format(outputDir, metric, datetime.datetime.now().strftime('%Y-%m-%d-%H-%M-%S'))
...
def plotTimeSeriesSeaborn(metric, data, outputDir='', xlabel=None):
    ...
    for experiment, experimentData in data.iteritems():
        ...
        name = '{0}/timeseries-{1}-{2}-{3}.png'.format(outputDir, metric, experiment, datetime.datetime.now().strftime('%Y-%m-%d-%H-%M-%S'))
        plt.xlabel(getLabelName(metric))
        plt.tight_layout()
        plt.savefig(name)
        plt.close()
...
if __name__ == '__main__':
    ...
    resultTypes = ['paper', 'paper-modulation-analysis', 'paper-ilp-throughput', 'paper-airtime', 'distance', 'modulation-distance', 'modulation-omega', 'modulation-slotframe', 'cdf', 'time', 'other', 'manualplot', 'modulation-ss', 'modulation-ilp', 'modulation-analysis']
    ...
    for experimentType in experimentTypes:
        ...
        reliability = reliability.append(calculateReliability(data[experimentType], experimentType, modulationConfig=modulationConfig, ss=slotframeSize))
        ...
        received = received.append(calculateReceived(data[experimentType], experimentType, nrMotes=nrMotes, modulationConfig=modulationConfig,ss=slotframeSize))
        pktArrivedToGenerated = pktArrivedToGenerated.append(calculatePktArrivedToGen(data[experimentType], experimentType, nrMotes=nrMotes, modulationConfig=modulationConfig, ss=slotframeSize))
